...
Each tag is a multi-arch manifest for `linux/amd64` and `linux/arm64`. `docker pull` selects the matching architecture automatically.
...
1. `docker-publish.yml` – builds `linux/amd64` and `linux/arm64` images on pushes, PRs, and version tags; pushes a multi-arch manifest on non‑PR events.
